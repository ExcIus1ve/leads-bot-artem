...
def build_report(start_date, end_date):
    ...
    try:
        ...
        report += (
            ...
            f"Лидов: {total_leads}\n"
            f"Стоимость лида: {total_cpa:.0f} ₽\n"
            f"Потрачено: {total_budget:,.0f} ₽"
        )

        return report
    except Exception as e:
        return f"❌ Ошибка при сборе отчёта: {e}"
...
def handle_stats(message):
    ...
    if len(args) == 3:
        try:
            ...
            bot.send_message(
                message.chat.id,
                "❌ Неверный формат. Используйте: /stats 2025-11-01 2025-11-30"
            )
    else:
        bot.send_message(
            message.chat.id,
            "📊 Формат: /stats 2025-11-01 2025-11-30"
        )
...
@bot.message_handler(
    func=lambda m: m.text and any(
        word in m.text.lower()
        for word in ['заявка', 'коттедж', 'дом', 'построить', 'смета', 'проект']
    )
)
def catch_lead(message):
    ...
    if 'yandex' in text or 'директ' in text:
        source = 'Яндекс.Директ'
    elif 'vk' in text or 'вк' in text:
        source = 'VK Реклама'
    ...
    try:
        ...
        bot.reply_to(message, f"❌ Ошибка при записи лида: {e}")
...
def send_weekly_report():
    ...
    try:
        ...
        print(f"❌ Ошибка при отправке еженедельного отчёта: {e}")
...
def send_monthly_report():
    ...
    try:
        ...
        print(f"❌ Ошибка при отправке ежемесячного отчёта: {e}")
...
def schedule_jobs():
    ...
    schedule.every().day.at("10:00").do(
        lambda: send_monthly_report() if datetime.date.today().day == 1 else None
    )
